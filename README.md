...
The library can be added to a sketch by including the lines in the example below. This example is for the API 5 library for BT820,
...
The `#include <BridgetekEVE5.h>` line will add in the header for this library and the declarations and constants included in the library API.

The `BridgetekEVE5 eve` creates a class for the library which is required to direct accesses to the EVE device to the library.
...
The latest Programming Guide for the BT820 can be found on the [Bridgetek Programming Guides](https://brtchip.com/document/programming-guides/) page.
...
A simple program for Arduino would add a set of display list items to the screen. This example will write the phrase "Testing BT820"
...
The `simple_5` example is an ideal sketch to start with as it shows many aspects of writing a sketch for Arduino.
